...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function signIn(username: string, password: string) {
    ...
    // persistencia segura (main)
    await window.api.auth.setToken(res.access_token)
    setToken(res.access_token)
    setMemToken(res.access_token)

    // confirma roles desde /auth/me (source of truth del token)
    const u = await apiMe().catch(() => null)
    setUser(u ? { ...u, safeUser: res.user } : { id: res.user.id, username: res.user.username, fullName: res.user.fullName, roles: res.user.roles, safeUser: res.user })
  }
